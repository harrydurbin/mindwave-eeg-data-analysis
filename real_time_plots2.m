...
clear all
close all
test_time = 1; %predicted time (in minutes) of
...
l = 1024; %determines length of signal to fft and plot
...
while (i < 512*60*test_time)   %stop when record is full
    ...
    if (j == 256) && (i<512*60*test_time-512)
...
%         PowerSpectrum(hax, data_raw(k:k+l-1), 512);
        figure(1)
        pwelch(data_raw(k:k+l-1), [], [], 1:40, 512)
        ylim([0 50])
        figure(2)
        pyulear(data_raw(k:k+l-1), 4, 1:40, 512)
        ylim([0 50])
        drawnow
        k = k + 256;
        j = 0;
...
save('data_raw_15_a.mat', data_raw)
